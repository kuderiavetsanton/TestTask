...
const SinglePost: React.FC<PropsType> = ({ post }) => {
    ...
    const singlePost: Post = state.posts.find(
    ...
    const handleCreateComment = (e: FormEvent<HTMLFormElement>) => {
        ...
        dispatch(
            createCommentThunk({ postId: singlePost.id, body: commentBody })
        )
        setCommentBody('')
    }
        ...
        <Container>
            {!!singlePost ? (
                <div>
                    {isEdit ? (
                        <PostModify
                            bodyInit={singlePost.body}
                            titleInit={singlePost.title}
                            modifyName="Edit"
                            dispatchAction={editPostThunk(singlePost.id)}
                        />
                    ) : (
                        <Card>
                            ...
                            <ButtonMain onClick={handleDelete}>
                                Delete
                            </ButtonMain>
                        </Card>
                    )}
